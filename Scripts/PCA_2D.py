...
for year in years:
    ...
    params = define_filenames_2D(params, model_name)
    ...
    for ax, kernel in zip(axs, dataset["kernels"]):
        title = dataset["kernels"][kernel]["Name"]
        pca.create(dataset["2D"]["components"], kernel)
        # Calculo de PCA
        pca.run(data.embedding)
        vectors = pca.get_eigenvectors()
        data.add_results(vectors, pca.names)
