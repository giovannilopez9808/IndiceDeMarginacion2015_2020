...
for year in years:
    ...
    frecuency.index = Categorical(frecuency.index, params["classes"].keys())
    frecuency = frecuency.sort_index()
    print(frecuency.index)
    plt.subplots(figsize=(10, 4))
    bars = plt.bar(frecuency.index, frecuency.values, color=colors.values())
    autolabel(bars)
    plt.xlabel("Grado de marginación")
    plt.ylabel("Porcentaje de municipios")
    print(max(frecuency))
    plt.ylim(0, 40)
    plt.grid(ls="--", alpha=0.5, color="#000000", axis="y")
    plt.tight_layout()
